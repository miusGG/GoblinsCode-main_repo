        ...
        for i in range(4):
            if i in world_map[str(c[0])][str(c[1])][2]:
                for j in range(len(dC[i])):
                    collisions.append(
                        [(dC[i][j][0] + c[0] * roomsize, dC[i][j][1] + c[1] * roomsize, dC[i][j][2], dC[i][j][3]),
                         "stop"])
                if (fromr == None) or ((fromr != None) and (i != doors2[fromr][3])):
                    collisions.append(
                        [(gC[i][0] + c[0] * roomsize, gC[i][1] + c[1] * roomsize, gC[i][2], gC[i][3]), "generate",
                         [c, i]])
            else:
                collisions.append(
                    [(wC[i][0] + c[0] * roomsize, wC[i][1] + c[1] * roomsize, wC[i][2], wC[i][3]), "stop"])
